...
function _onChkobbaTableCardTap(tableCardEl) {
    ...
    if (match) {
        ...
        // Auto-commit capture without confirmation prompt
...
function _refreshChkobbaCaptureHighlights() {
    ...
    if (!ctx || !_chkobbaPlaySession) return;
    const { playedCard, selectedTableIds } = _chkobbaPlaySession;
    // Removed hints - no longer showing capture candidates
...
function _renderChkobbaTopPlayers(state, me) {
    const scrollCont = document.getElementById('chkobba-players-scroll');
    if (!scrollCont) return;
    
    const currentPlayer = state.players[state.turnIndex];
    document.getElementById('cti-player-name').textContent = currentPlayer ? _esc(currentPlayer.name) : '...';
    
    scrollCont.innerHTML = '';
    state.players.forEach(p => {
        const isTurn = state.players[state.turnIndex].id === p.id;
        const pill = document.createElement('div');
        pill.className = `chkobba-player-scroll-pill${isTurn ? ' is-turn' : ''}`;
        pill.innerHTML = `${_esc(p.name)} (${p.hand.length})`;
        scrollCont.appendChild(pill);
    });
}
...
function _renderChkobbaInfoPills(state, me) {
    ...
    infoCont.innerHTML = `
        <div class="chkobba-my-stats">
            <div class="chkobba-my-avatar">😊</div>
            <div class="chkobba-my-name">${_esc(me.name)}</div>
        </div>
        <div class="chkobba-stats-grid">
            <div class="chkobba-stat-item">
                <span class="stat-icon">🃏</span>
                <span class="stat-label">كوارط</span>
                <span class="stat-value">${captured.length}</span>
            </div>
            <div class="chkobba-stat-item">
                <span class="stat-icon">♦️</span>
                <span class="stat-label">ديناري</span>
                <span class="stat-value">${dinari}</span>
            </div>
            <div class="chkobba-stat-item">
                <span class="stat-icon">🏆</span>
...
function _showOnlineChkobba(room) {
    ...
    // Menu button actions
    document.getElementById('chkobba-back-to-menu-btn')?.addEventListener('click', () => {
        menuDropdown?.classList.remove('show');
    ...
    document.getElementById('chkobba-reconnect-btn')?.addEventListener('click', () => {
        menuDropdown?.classList.remove('show');
        if (window.onlineMode && _room) {
            _subscribe(_room.code);
        }
    });

    // Setup voice chat button
...
function _showTournamentBracket(room) {
    ...
    // Clear styles that might interfere with CSS aspect-ratio
    // Do not set aspectRatio, width, or height - let CSS handle layout

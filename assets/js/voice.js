...
function _buildVoiceUI(){document.getElementById('voice-panel')?.remove();const panel=document.createElement('div');panel.id='voice-panel';panel.innerHTML=`<div class="vc-live"><span class="vc-live-dot"></span>صوت</div><div class="vc-sep"></div><button id="voice-mute-btn" title="كتم/فتح الميكروفون">🎙️</button><button id="voice-leave-btn" title="قطع الصوت">✕</button>`;const shell=document.getElementById('app-shell')||document.body;shell.appendChild(panel);document.getElementById('voice-mute-btn').addEventListener('click',toggleMute);document.getElementById('voice-leave-btn').addEventListener('click',stopVoice);_makeDraggable(panel);}
...
function _makeDraggable(el){let startX,startY,initialX,initialY;el.addEventListener('pointerdown',e=>{if(e.target.closest('button'))return;startX=e.clientX;startY=e.clientY;const rect=el.getBoundingClientRect();const shell=document.getElementById('app-shell')||document.body;const shellRect=shell.getBoundingClientRect();const scale=window._gameScale||1;initialX=(rect.left-shellRect.left)/scale;initialY=(rect.top-shellRect.top)/scale;el.setPointerCapture(e.pointerId);el.addEventListener('pointermove',move);el.addEventListener('pointerup',up);el.addEventListener('pointercancel',up);});function move(e){const dx=(e.clientX-startX)/(window._gameScale||1);const dy=(e.clientY-startY)/(window._gameScale||1);el.style.left=(initialX+dx)+'px';el.style.top=(initialY+dy)+'px';el.style.bottom='auto';el.style.transform='none';}function up(e){el.removeEventListener('pointermove',move);el.removeEventListener('pointerup',up);el.removeEventListener('pointercancel',up);}}

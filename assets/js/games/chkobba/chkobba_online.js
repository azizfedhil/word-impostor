...
// When a render is requested mid-animation we store the latest room here.
// After the animation ends, _chkobbaSetAnimating(false) fires the pending render.
let _chkobbaPendingRoom = null;

/**
 * Central setter for _chkobbaAnimating.
 * When clearing (false), flushes any render that arrived during the animation
 * so the UI is always up-to-date once motion completes.
 */
function _chkobbaSetAnimating(value) {
    _chkobbaAnimating = value;
    if (!value && _chkobbaPendingRoom) {
        const room = _chkobbaPendingRoom;
        _chkobbaPendingRoom = null;
...
function _resetChkobbaPlaySession() {
    ...
    // Restore any cards that were hidden (opacity:0) for a flight animation
    // that may have been interrupted or whose render was deferred.
    document.querySelectorAll('#chkobba-my-hand .hand-card, #chkobba-table .table-card').forEach(el => {
...
function _showOnlineChkobba(room) {
    ...
    // The latest room is stored; _chkobbaSetAnimating(false) will replay it
    // once the animation finishes so the UI never gets stuck stale.
    if (_chkobbaAnimating) {
        _chkobbaPendingRoom = room;
        return;
    }
    ...
    state.table.forEach((card, idx) => {
        ...
        if (!cardEl) {
            // Genuinely new card — create it and mark for entry animation
            cardEl = _renderChkobbaCard(card, {
                zone: 'table',
                index: idx,
                interactive: false   // interactivity is handled by delegation
            });
            // .is-new-card triggers tableCardEnter once; remove after it plays
            // so re-renders never re-fire the animation on an existing card
            cardEl.classList.add('is-new-card');
            cardEl.addEventListener('animationend', () => cardEl.classList.remove('is-new-card'), { once: true });
    ...
    const renderHand = () => {
        ...
        // When interactivity changes (turn boundary), remove ALL existing hand cards
        // so that fresh elements with correct drag/pointer listeners are created below.
        // Without this, old card elements remain in the DOM alongside newly-created ones,
        // causing duplicate cards to appear (e.g. 4 cards shown instead of 2).
        if (handInteractChanged) {
            existingHandEls.forEach(el => el.remove());
        } else {
            existingHandEls.forEach(el => {
        ...
        if (me) {
            me.hand.forEach((card, idx) => {
                ...
                // When interactivity changed we already wiped the old elements above,
                // so existingHandById will always miss — that's intentional.
    ...
    if (shouldDealAnim) {
        ...
        // Fade out any existing hand cards smoothly before the deal flies in,
        // rather than clearing innerHTML which causes an instant blank flash.

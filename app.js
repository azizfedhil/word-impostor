...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function _resizeGame() {
        ...
        if (!shell) return;
        // Read the container's actual constrained dimensions after CSS aspect-ratio
        const containerWidth = shell.clientWidth;
        const containerHeight = shell.clientHeight;
        // Set scale based on actual container size relative to base 480px width
        const factor = containerWidth / 480;
        window._gameScale = factor;
        // Do not override CSS layout - let CSS handle aspect-ratio and sizing
        // shell.style.transform is not set to preserve CSS aspect-ratio constraint
        // document.body.style.minHeight is not set to preserve CSS overflow behavior
    }

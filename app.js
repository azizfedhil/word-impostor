...
let currentGameMode = (typeof window !== 'undefined' && window.__DAKHEEL_GAME_MODE) || 'impostor';
const gameModes = {
...
function setGameMode(mode, goSetup = true) {
    if (window.__DAKHEEL_FIXED_MODE && window.__DAKHEEL_GAME_MODE) {
        mode = window.__DAKHEEL_GAME_MODE;
    }
    ...
    updateGameModeUI();
    if (goSetup) {
        if (window.__DAKHEEL_ONLINE_ONLY || currentGameMode === 'coup' || currentGameMode === 'chkobba') showScreen('online-setup-screen');
        else if (document.getElementById('setup-screen')) showScreen('setup-screen');
    }
}
window.__legacySetGameMode = setGameMode;
if (!window.__DAKHEEL_FIXED_MODE) {
    window.setGameMode = setGameMode;
    window.getCurrentGameMode = () => currentGameMode;
} else {
    window.setGameMode = (m, go) => { if (m === window.__DAKHEEL_GAME_MODE) setGameMode(m, go); };
    window.getCurrentGameMode = () => window.__DAKHEEL_GAME_MODE;
}
...
function updateGameModeUI() {
    ...
    const plugin = window.__DAKHEEL_SOCIAL_PLUGIN;
    const voteBtn = document.getElementById('go-to-vote-btn');
    if (voteBtn) {
        voteBtn.innerText = plugin?.voteButtonText
            || (currentGameMode === 'thief' ? '⚖️ يا حاكم، احكم' : currentGameMode === 'spyfall' ? '🕶️ عرفنا الspy' : i18n[currentLang].vote_btn);
    }
    const who = document.querySelector('[data-i18n="who_impostor"]');
    if (who) {
        who.innerText = plugin?.whoVoteText
            || (currentGameMode === 'thief' ? 'يا حاكم، شكون السارق؟' : currentGameMode === 'spyfall' ? 'شكون الspy؟' : i18n[currentLang].who_impostor);
    }
...
window.__legacyApplyGameModeUI = updateGameModeUI;
...
const _assetRoot = () => window.__DAKHEEL_ASSET_ROOT || '';
const _gameModeForAssets = () => window.__DAKHEEL_GAME_MODE || currentGameMode;

if (!_gameModeForAssets() || _gameModeForAssets() === 'impostor') {
    fetch(_assetRoot() + 'word list.json', { cache: 'no-store' })
        .then(r => r.json()).then(d => { regularWordsDB = d; })
        .catch(() => { regularWordsDB = _embeddedRegular; });
}

if (!_gameModeForAssets() || _gameModeForAssets() === 'spyfall') {
    fetch(_assetRoot() + 'spyfall_tunisia_100_locations.json', { cache: 'no-store' })
...
function _assetUrl(path = '') {
    ...
    return `${window.__DAKHEEL_ASSET_ROOT || ''}${path}`;
}

function _coupCardIconHtml(card, cls = 'coup-card-avatar') {
    if (!card) return '';
    if (card.img) return `<img class="${cls}" src="${_escapeHtml(_assetUrl(card.img))}" alt="${_escapeHtml(card.name || '')}" loading="lazy">`;
    return `<span class="${cls}">${card.icon || ''}</span>`;
}

function _coupCardLargeHtml(card, cls = 'coup-card-portrait') {
    if (!card) return '';
    const src = _assetUrl(card.img512 || card.img);
...
async function bootApp() {
    ...
    const valTim = document.getElementById('val-timer');
    ...
    if (window.__DAKHEEL_FIXED_MODE) {
        if (window.__DAKHEEL_ONLINE_ONLY) showScreen('online-setup-screen');
        else if (document.getElementById('setup-screen')) showScreen('setup-screen');
    }
    ...
    document.getElementById('coup-guide-back-btn')?.addEventListener('click', () => showScreen(coupState ? 'coup-screen' : (document.getElementById('setup-screen') ? 'setup-screen' : 'online-setup-screen')));
    if (!window.__DAKHEEL_FIXED_MODE) {
    ...
    // COUNTERS
    document.getElementById('imp-minus')?.addEventListener('click',()=>{if(impostorConfig>1){impostorConfig--;const v=document.getElementById('val-impostors');if(v)v.innerText=impostorConfig;saveSettings();}});
    document.getElementById('imp-plus')?.addEventListener('click',()=>{impostorConfig++;const v=document.getElementById('val-impostors');if(v)v.innerText=impostorConfig;saveSettings();});
    document.getElementById('timer-minus')?.addEventListener('click',()=>{if(timerConfig>1){timerConfig--;const v=document.getElementById('val-timer');if(v)v.innerText=timerConfig;saveSettings();}});
    document.getElementById('timer-plus')?.addEventListener('click',()=>{timerConfig++;const v=document.getElementById('val-timer');if(v)v.innerText=timerConfig;saveSettings();});
